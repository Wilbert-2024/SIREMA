...
$filtro = 1;
$stmt = $con->conectar()->prepare('call reporte_ultima_actualizacion_de_registros_por_centro(?,?,?,?,?)');
...
foreach ($registro as $reg)
{
    ...
    $pdf->Cell(60, 5, $reg['Centro'], 0, 0, 'C', 0);
    $pdf->Cell(60, 5, $reg['TI'], 0, 0, 'C', 0);
    $pdf->Cell(60, 5, $reg['Sem'], 0, 0, 'C', 0);

    $stmt = $con->conectar()->prepare('call reporte_ultima_actualizacion_de_registro_por_centro_detalle(?,?,?,?,?)');
    $stmt->bindParam(1,$_SESSION['usuario'] ,PDO::PARAM_STR);
    $stmt->bindParam(2,$reg['Id'] ,PDO::PARAM_INT);
    $stmt->bindParam(3,$_POST['TipoIngresoId'] ,PDO::PARAM_INT);
    $stmt->bindParam(4,$_POST['SemestreId'] ,PDO::PARAM_INT);
    $stmt->bindParam(5,$_POST['AnioLectivoId'] ,PDO::PARAM_INT);
    $stmt->execute();
    ...
    foreach ($ARCS as $a)
    {
        ...
        foreach ($detalles as $det)
        {
            ...
            if($a == $det['ARC'])
            {
                ...
                if(!$areaSet)
                {
                    $pdf->Cell(175, 5, $det['AreaC'], 1, 0, 'C', 1);
                    $areaSet = true;
                    $pdf->Ln(5);
                    $pdf->SetFont('helvetica', 'B', 8);
                    $pdf->SetFillColor(224, 235, 255);
                    $pdf->Cell(5, 5, 'Año', 1, 0, 'L', 1);
                    $pdf->Cell(60, 5, 'Carrera', 1, 0, 'L', 1);
                    $pdf->Cell(20, 5, 'Turno', 1, 0, 'L', 1);
                    $pdf->Cell(10, 5, 'Grupo', 1, 0, 'L', 1);
                    $pdf->Cell(15, 5, 'FEM', 1, 0, 'L', 1);
                    $pdf->Cell(15, 5, 'MAS', 1, 0, 'L', 1);
                    $pdf->Cell(15, 5, 'Total', 1, 0, 'L', 1);
                    $pdf->Cell(35, 5, 'Actualizado', 1, 0, 'L', 1);
                ...
                $pdf->Cell(5, 5, $det['anio'], 1, 0, 'L', 1);
                $pdf->Cell(60, 5, $det['Carrera'], 1, 0, 'L', 1);
                $pdf->Cell(20, 5, $det['turno'], 1, 0, 'L', 1);
                $pdf->Cell(10, 5, $det['grupo'], 1, 0, 'L', 1);
                $pdf->Cell(15, 5, $det['fem'], 1, 0, 'L', 1);
                $pdf->Cell(15, 5, $det['mas'], 1, 0, 'L', 1);
                $pdf->Cell(15, 5, $det['TotalR'], 1, 0, 'L', 1);
                $pdf->Cell(35, 5, $det['Fecha'], 1, 0, 'L', 1);
    ...
    $pdf->Cell(95, 4, 'TOTAL ** '.$reg['Centro'], 0, 0, 'L');
...
$pdf->Ln(13);
$pdf->SetFont('helvetica', 'B', 13);
$pdf->Cell(95, 4, 'TOTAL GENERAL ** ', 0, 0, 'L');
